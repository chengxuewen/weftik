#!/usr/bin/env python3
"""audesys — AUDESYS 统一构建 CLI（单入口）。

薄壳（audesys.sh/.bat）保证 pixi 环境激活后调用本脚本；
环境内 PATH 已注入，subprocess 直接调 cargo 等。
平台差异仅 clean（rmdir vs rmtree）与 run（pkill Unix-only）两处。

用法: audesys [-h] {build,test,qa,run,deploy,clean,config,status,version} ...
"""
...
def _find_runtime_binary() -> Path | None:
    ...
    if cargo_target:
        candidates.append(Path(cargo_target) / "debug/audesys-runtime")
    candidates += [
        ROOT / "target/debug/audesys-runtime",
        ROOT / "target/release/audesys-runtime",
    ]
...
def _cmd_run(foreground: bool) -> None:
    ...
    if bin_path is None:
        print("错误: 未找到 audesys-runtime 二进制 — 先运行: audesys build", file=sys.stderr)
        sys.exit(1)
    subprocess.run(["pkill", "-x", "audesys-runtime"], check=False)  # no-op if none
    ...
    log_path = Path("/tmp/audesys-runtime.log")
    ...
    print(f"✓ audesys-runtime 已启动 (PID {proc.pid}) — 日志: {log_path}")
...
def main() -> None:
    ...
    run_p = sub.add_parser("run", help="启动 audesys-runtime [-f 前台实时日志]（macOS/Linux）")
