...
│  └─────────┴──────────┴────────┴───────┴─────────────┘  │
├─────────────────────────────────────────────────────────┤
│  工具层: MCP (codegraph/playwright/openspace/github) + LSP (typescript-language-server) + Skills│
├─────────────────────────────────────────────────────────┤
...
### 2.3 oh-my-opencode (OMO) — Agent 编排核心
...
| `local-playwright` | Phase 1+ | 浏览器自动化测试 | ✅ |
| `local-github` | 全部阶段 | GitHub 代码搜索、仓库探索（需 `GITHUB_TOKEN`） | ✅ |
| `local-openspace` | Phase 0+（开发辅助） | AI agent 技能进化引擎（需 API Key） | ✅ |
| `local-memory` | Phase 0+（可选） | 持久化记忆系统 | ❌ |
| `local-postgres` | Phase 2（DB 就绪） | PostgreSQL schema/query/monitor | ❌ |
#### 阶段说明

- **P0（文档/CI）**: codegraph、github、openspace（AI 辅助），memory（可选）
- **P1（前端/HAL）**: remote-qt-docs（Qt 文档）、playwright（浏览器测试）
- **P2（DB 就绪）**: postgres（数据库管理），当前为 ❌
...
| MCP | 排除原因 |
|---|---|
| `shadcn/tailwind/lucide` (3 个 MCP) | Phase 0 零前端代码，无产出（D42） |
| `context7` | 已被 librarian agent 的 Context7 集成覆盖 |
...
1. **compact 后自动恢复** — context-mode 自动重建状态
2. **用 explore/librarian 代替直接 grep** — 分派后台 agent 并行搜索
3. **分派独立子任务** — 用 `task(run_in_background=true)` 隔离上下文，子 agent 完成后自动清理
