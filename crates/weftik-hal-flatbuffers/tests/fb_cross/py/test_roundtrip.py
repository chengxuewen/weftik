#!/usr/bin/env python3
"""Weftik FlatBuffers Python roundtrip + cross-language tests.
...
import flatbuffers
from weftik.hal.HalType import HalType
from weftik.hal.HalValueData import HalValueData
from weftik.hal.HalSignal import (
    HalSignal,
    HalSignalStart,
    HalSignalAddName,
    HalSignalAddValue,
...
from weftik.hal.HalValue import (
    HalValue,
    HalValueStart,
    HalValueAddType,
    HalValueAddDataType,
...
from weftik.hal.BoolVal import BoolValStart, BoolValAddValue, BoolValEnd
from weftik.hal.S32Val import S32ValStart, S32ValAddValue, S32ValEnd
from weftik.hal.F64Val import F64ValStart, F64ValAddValue, F64ValEnd
from weftik.hal.StringVal import StringValStart, StringValAddValue, StringValEnd
from weftik.hal.BlobVal import (
    BlobValStart,
    BlobValAddData,
    BlobValEnd,
    BlobValStartDataVector,
)
from weftik.hal.S8Val import S8ValStart, S8ValAddValue, S8ValEnd
from weftik.hal.U8Val import U8ValStart, U8ValAddValue, U8ValEnd
from weftik.hal.S16Val import S16ValStart, S16ValAddValue, S16ValEnd
from weftik.hal.U16Val import U16ValStart, U16ValAddValue, U16ValEnd
from weftik.hal.U32Val import U32ValStart, U32ValAddValue, U32ValEnd
from weftik.hal.S64Val import S64ValStart, S64ValAddValue, S64ValEnd
from weftik.hal.U64Val import U64ValStart, U64ValAddValue, U64ValEnd
from weftik.hal.F32Val import F32ValStart, F32ValAddValue, F32ValEnd
from weftik.hal.ArrayData import (
    ArrayDataStart,
...
def value_from_halvalue(hv):
    ...
    if dt == HalValueData.bool_val:
        ...
        from weftik.hal.ArrayData import ArrayData
        av = ArrayData()
        ...
        from weftik.hal.S8Val import S8Val
        sv = S8Val()
        sv.Init(data.Bytes, data.Pos)
        return ("s8", sv.Value())

    elif dt == HalValueData.u8_val:
        from weftik.hal.U8Val import U8Val
        uv = U8Val()
        uv.Init(data.Bytes, data.Pos)
        return ("u8", uv.Value())

    elif dt == HalValueData.s16_val:
        from weftik.hal.S16Val import S16Val
        sv = S16Val()
        sv.Init(data.Bytes, data.Pos)
        return ("s16", sv.Value())

    elif dt == HalValueData.u16_val:
        from weftik.hal.U16Val import U16Val
        uv = U16Val()
        uv.Init(data.Bytes, data.Pos)
        return ("u16", uv.Value())

    elif dt == HalValueData.u32_val:
        from weftik.hal.U32Val import U32Val
        uv = U32Val()
        uv.Init(data.Bytes, data.Pos)
        return ("u32", uv.Value())

    elif dt == HalValueData.s64_val:
        from weftik.hal.S64Val import S64Val
        sv = S64Val()
        sv.Init(data.Bytes, data.Pos)
        return ("s64", sv.Value())

    elif dt == HalValueData.u64_val:
        from weftik.hal.U64Val import U64Val
        uv = U64Val()
        uv.Init(data.Bytes, data.Pos)
        return ("u64", uv.Value())

    elif dt == HalValueData.f32_val:
        from weftik.hal.F32Val import F32Val
        fv = F32Val()
        fv.Init(data.Bytes, data.Pos)
        return ("f32", fv.Value())
