...
pub fn halvalue_to_variables(
    value: &weftik_hal_core::HalValue,
) -> Result<(Vec<u8>, VariableType), super::error::HartError> {
    match value {
        weftik_hal_core::HalValue::F32(v) => Ok((encode_f32_be(*v).to_vec(), VariableType::Float)),
        weftik_hal_core::HalValue::U8(v) => Ok((vec![*v], VariableType::U8)),
        weftik_hal_core::HalValue::U16(v) => Ok((v.to_be_bytes().to_vec(), VariableType::U16)),
        weftik_hal_core::HalValue::U32(v) => {
