//! Weftik G-code compiler — ISO 6983 / RS274 → HAL IR.
//!
//! Compiles RS274 G-code source text into a [HalProgram] for execution
//! on the Weftik register VM. Supports:
//!
    ...
    fn test_g0_rapid() {
        ...
        assert!(
            program
                .instructions
                .iter()
                .any(|inst| inst.opcode == weftik_hal_ir::instruction::Opcode::Store)
        );
    }
...
M30";
    ...
    fn test_modal_switching() {
        ...
        let store_count = program
            .instructions
            .iter()
            .filter(|inst| inst.opcode == weftik_hal_ir::instruction::Opcode::Store)
            .count();
    ...
    fn test_m30_with_spindle_off() {
        ...
        assert!(
            program
                .instructions
                .iter()
                .any(|inst| inst.opcode == weftik_hal_ir::instruction::Opcode::Halt)
        );
    }
...
M30       ; end";
    ...
    fn test_g1_step_per_cycle() {
        ...
        let has_jump_if = program
            .instructions
            .iter()
            .any(|inst| inst.opcode == weftik_hal_ir::instruction::Opcode::JumpIf);
