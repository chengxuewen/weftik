...
#[allow(dead_code)]
const REG_COUNTER: u8 = 1; // r1 = cycle counter (used by weftik-cnc-motion)
...
// REG_VEL(7) through REG_DT(12) moved to weftik-cnc-motion
...
pub fn compile_commands(
    ...
    // ponytail: dt is loaded per motion by weftik-cnc-motion crate
...
fn emit_arc(
    ...
    for i in 1..=num_segments {
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(a_signal.into()),
                weftik_hal_ir::types::Operand::Register(REG_POS_X),
            ],
        ));
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(enable_a.into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(b_signal.into()),
                weftik_hal_ir::types::Operand::Register(REG_POS_X),
            ],
        ));
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(enable_b.into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(linear_signal.into()),
                weftik_hal_ir::types::Operand::Register(REG_POS_X),
            ],
        ));
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName(enable_linear.into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
    }
...
fn emit_g0(state: &ModalState, instructions: &mut Vec<Instruction>) {
    ...
    instructions.push(Instruction::new(
        Opcode::Store,
        vec![
            weftik_hal_ir::types::Operand::SignalName("axis.0.pos".into()),
            weftik_hal_ir::types::Operand::Register(REG_POS_X),
        ],
    ));
    // Enable axis
    ...
    instructions.push(Instruction::new(
        Opcode::Store,
        vec![
            weftik_hal_ir::types::Operand::SignalName("axis.0.enable".into()),
            weftik_hal_ir::types::Operand::Register(REG_STEP),
        ],
    ));
    ...
    if state.current_y != 0.0 {
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName("axis.1.pos".into()),
                weftik_hal_ir::types::Operand::Register(REG_POS_Y),
            ],
        ));
        instructions.push(Instruction::load_imm(REG_STEP, HalValue::Bool(true)));
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName("axis.1.enable".into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
    }
    ...
    if state.current_z != 0.0 {
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName("axis.2.pos".into()),
                weftik_hal_ir::types::Operand::Register(REG_POS_Z),
            ],
        ));
        instructions.push(Instruction::load_imm(REG_STEP, HalValue::Bool(true)));
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName("axis.2.enable".into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
    }
...
// moved to weftik-cnc-motion crate
...
fn emit_spindle(
    ...
    if let Some(s) = cmd.s {
        ...
        instructions.push(Instruction::new(
            Opcode::Store,
            vec![
                weftik_hal_ir::types::Operand::SignalName("spindle.rpm".into()),
                weftik_hal_ir::types::Operand::Register(REG_STEP),
            ],
        ));
    }
    ...
    match cmd.m_code {
        Some(3) => {
            ...
            instructions.push(Instruction::new(
                Opcode::Store,
                vec![
                    weftik_hal_ir::types::Operand::SignalName("spindle.cw".into()),
                    weftik_hal_ir::types::Operand::Register(REG_STEP),
                ],
            ));
        }
        Some(4) => {
            ...
            instructions.push(Instruction::new(
                Opcode::Store,
                vec![
                    weftik_hal_ir::types::Operand::SignalName("spindle.ccw".into()),
                    weftik_hal_ir::types::Operand::Register(REG_STEP),
                ],
            ));
        }
...
mod tests {
    ...
    fn test_g0_single_axis() {
        ...
        let has_store = program.instructions.iter().any(|inst| {
            inst.opcode == Opcode::Store
                && inst.operands.first()
                    == Some(&weftik_hal_ir::types::Operand::SignalName("axis.0.pos".into()))
        });
        assert!(has_store);
    }
    ...
    fn test_g0_multi_axis() {
        ...
        let has_y = program.instructions.iter().any(|inst| {
            inst.opcode == Opcode::Store
                && inst.operands.first()
                    == Some(&weftik_hal_ir::types::Operand::SignalName("axis.1.pos".into()))
        });
        let has_z = program.instructions.iter().any(|inst| {
            inst.opcode == Opcode::Store
                && inst.operands.first()
                    == Some(&weftik_hal_ir::types::Operand::SignalName("axis.2.pos".into()))
        });
        assert!(has_x);
        assert!(has_y);
        assert!(has_z);
    ...
    fn test_m3_spindle_cw() {
        ...
        let has_cw = program.instructions.iter().any(|inst| {
            inst.opcode == Opcode::Store
                && inst.operands.first()
                    == Some(&weftik_hal_ir::types::Operand::SignalName("spindle.cw".into()))
        });
        assert!(has_cw);
    }
