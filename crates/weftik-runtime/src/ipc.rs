    ...
    loop {
        ...
        match method_id {
            ...
            METHOD_WRITE_SIGNAL => {
                ...
                let mw = engine.middleware();
                let mw_guard = mw.lock().unwrap();
                let ts = weftik_hal_core::types::Timestamp {
                    secs: now_ms() / 1000,
            ...
            METHOD_PAUSE => {
                ...
                engine.pause();
                let _ = write_all(&mut stream, &build_ok_response(METHOD_PAUSE, b"paused"));
            }
            ...
            METHOD_RESUME => {
                ...
                engine.resume();
                let _ = write_all(&mut stream, &build_ok_response(METHOD_RESUME, b"resumed"));
            }
            ...
            METHOD_STEP_CYCLE => {
                ...
                engine.step_cycle();
                let _ = write_all(&mut stream, &build_ok_response(METHOD_STEP_CYCLE, b"stepped"));
            }
            ...
            METHOD_LIST_BREAKPOINTS => {
                ...
                let hal_exec = engine.hal_executor();
                let guard = hal_exec.read().unwrap();
                let response = match guard.as_ref() {
                    Some(executor) => {
                        let bps = executor.list_breakpoints();
                        bps.iter().map(|ip| ip.to_string()).collect::<Vec<_>>().join(",")
                    }
                    None => {
                        drop(guard);
                        let _ = write_all(
            ...
            METHOD_READ_REGISTERS => {
                ...
                let response = match guard.as_ref() {
                    Some(executor) => {
                        let val = executor.vm().read_register(idx);
                        format!("r{}={:?}", idx, val)
                    }
                    None => {
                        drop(guard);
            ...
            METHOD_DEBUG_STATE => {
                ...
                let hal_exec = engine.hal_executor();
                let guard = hal_exec.read().unwrap();
                let (debug_mode, bps, trace_len) = match guard.as_ref() {
                    Some(executor) => (
                        executor.is_debug_enabled(),
                        executor.list_breakpoints(),
                        executor.trace_buffer().len() as u64,
                    ),
                    None => (false, Vec::new(), 0u64),
                };
                drop(guard);
...
mod tests {
    ...
    fn test_new_server_not_running() {
        ...
        let server = IpcServer::new("/tmp/test-weftik-new.sock", Arc::new(engine));
        assert!(!server.is_running());
    }

    #[test]
    fn test_ipc_server_start_stop() {
        let path = "/tmp/test-weftik-ipc-lifecycle.sock";
    ...
    fn test_uds_client_can_connect() {
        ...
        let path = "/tmp/test-weftik-connect.sock";
        let _ = std::fs::remove_file(path);
        let server = IpcServer::new(path, Arc::new(test_engine()));
        let h = server.start().expect("start");
        std::thread::sleep(Duration::from_millis(50));
    ...
    fn test_health_unauth_rejected() {
        use std::io::Write;
        use std::os::unix::net::UnixStream;
        let path = "/tmp/test-weftik-health.sock";
        let _ = std::fs::remove_file(path);
        let server = IpcServer::new(path, Arc::new(test_engine()));
        let h = server.start().expect("start");
        std::thread::sleep(Duration::from_millis(50));
    ...
    fn test_auth_flow_sends_token() {
        ...
        let path = "/tmp/test-weftik-auth-flow.sock";
        let _ = std::fs::remove_file(path);
        let server = IpcServer::new(path, Arc::new(test_engine()));
        let h = server.start().expect("start");
        std::thread::sleep(Duration::from_millis(50));
    ...
    fn test_uds_response_format() {
        ...
        let path = "/tmp/test-weftik-response.sock";
        let _ = std::fs::remove_file(path);
        let server = IpcServer::new(path, Arc::new(test_engine()));
        let h = server.start().expect("start");
        std::thread::sleep(Duration::from_millis(50));
    ...
    fn test_engine() -> Engine {
        ...
        impl weftik_hal_core::qos::HalQoS for DummyMw {
            fn lock_level(&self) -> weftik_hal_core::qos::LockLevel {
                weftik_hal_core::qos::LockLevel::None
            }
            fn set_lock_level(
                &self,
                _: weftik_hal_core::qos::LockLevel,
        ...
        impl weftik_hal_core::middleware::AmwMiddleware for DummyMw {
