//! Structured JSON-line logger for the Weftik controller.
//!
...
//! # Configuration
//!
//! Set `WEFTIK_LOG_LEVEL` environment variable: `error`, `warn`, `info`, `debug`.
//! Default: `info`.
...
/// Initialize the logger. Reads WEFTIK_LOG_LEVEL from env.
pub fn init() {
    let level = std::env::var("WEFTIK_LOG_LEVEL").unwrap_or_else(|_| "info".to_string());
