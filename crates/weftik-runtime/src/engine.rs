...
mod tests {
    ...
    impl weftik_hal_core::discovery::HalDiscovery for MockMiddleware {
        fn list_all(&self) -> HalResult<Vec<weftik_hal_core::discovery::DiscoveryEntry>> {
            Ok(Vec::new())
        }
        fn find_by_name(
            &self,
            _name: &str,
        ) -> HalResult<Option<weftik_hal_core::discovery::DiscoveryEntry>> {
            Ok(None)
        }
        fn find_by_pattern(
            &self,
            _pattern: &str,
        ) -> HalResult<Vec<weftik_hal_core::discovery::DiscoveryEntry>> {
            Ok(Vec::new())
        }
        fn watch(
            &self,
            _cb: weftik_hal_core::discovery::WatchCallback,
        ) -> HalResult<weftik_hal_core::discovery::WatchHandle> {
            Ok(weftik_hal_core::discovery::WatchHandle {})
        }
    }
    ...
    impl HalTransport for MockMiddleware {
        ...
        fn subscribe_signal(
            ...
            _name: &str,
            _cb: SignalCallback,
        ) -> HalResult<weftik_hal_core::types::Subscription> {
            Ok(weftik_hal_core::types::Subscription {})
        }
        fn snapshot_signals(
            &self,
            _pattern: &str,
    ...
    impl HalQoS for MockMiddleware {
        ...
        fn check_liveliness(
            &self,
            _entity_id: &str,
        ) -> HalResult<weftik_hal_core::qos::LivelinessStatus> {
            Ok(weftik_hal_core::qos::LivelinessStatus::Alive)
        }
    ...
    fn test_signal_register_and_snapshot() {
        ...
        let def1 = SignalDef::new(
            "motor.speed",
            weftik_hal_core::types::HalPinType::F32,
            HalValue::F32(100.0),
            WriteStrategy::Own,
        );
    ...
    fn test_cycle_publishes_signals() {
        ...
        engine
            .register_signal(SignalDef::new(
                "out.status",
                weftik_hal_core::types::HalPinType::Bool,
                HalValue::Bool(true),
                WriteStrategy::Own,
            ))
            .unwrap();
    ...
    fn test_cycle_reads_monitored_signals() {
        ...
        engine
            .register_signal(SignalDef::new(
                "sensor.input",
                weftik_hal_core::types::HalPinType::U32,
                HalValue::U32(0),
                WriteStrategy::Monitored,
            ))
            .unwrap();
    ...
    fn test_swap_applied_at_cycle_boundary() {
        ...
        let instructions = vec![weftik_hal_ir::instruction::Instruction::halt()];
