...
use weftik_hal_core::{HalTransport, HalValue, Timestamp};
...
impl VirtualHARTDevice {
    ...
    ///          + device_type(0x01) + identifier string("Weftik")
    fn handle_cmd0_identifier(&self) -> Vec<u8> {
        ...
        resp.extend_from_slice(&[0x05, 0xDD]); // manufacturer ID: 0x05DD = Weftik
        resp.push(0x01); // device type
        resp.extend_from_slice(b"Weftik"); // device identifier
        resp
    }
...
impl Default for VirtualHARTDevice {
    fn default() -> Self {
        Self::new(0x01, "Weftik-VIRTUAL-HART")
    }
}
...
mod tests {
    ...
    fn test_default_device() {
        ...
        assert_eq!(dev.long_tag, "Weftik-VIRTUAL-HART");
    }
