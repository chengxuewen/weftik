...
//!   cargo build --bin weftik-runtime
//!   cargo build --bin weftik-agent
//!
...
// ── Wire constants — match crates/weftik-runtime/src/ipc.rs ────────────
...
fn controller_binary() -> String {
    if let Ok(p) = std::env::var("CARGO_BIN_EXE_weftik-runtime") {
        return p;
    }
    "./target/debug/weftik-runtime".into()
}

/// Path to the agent binary.
fn agent_binary() -> String {
    if let Ok(p) = std::env::var("CARGO_BIN_EXE_weftik-agent") {
        return p;
    }
    "./target/debug/weftik-agent".into()
}
...
const SOCKET_PATH: &str = "/tmp/weftik-runtime-test.sock";
...
fn test_controller_graceful_shutdown() {
    ...
    if ensure_binary("weftik-runtime", &bin_path).is_none() {
        return;
    }
...
fn test_agent_spawn_with_config() {
    ...
    if ensure_binary("weftik-runtime", &ctrl_bin).is_none()
        || ensure_binary("weftik-agent", &sup_bin).is_none()
    {
        return;
    }
    ...
    let config_path = "/tmp/weftik-agent-test.yaml";
    let config = format!(
