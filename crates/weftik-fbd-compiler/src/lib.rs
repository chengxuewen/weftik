//! Weftik IEC 61131-3 FBD (Function Block Diagram) compiler.
//!
//! Compiles CFC (Continuous Function Chart) text format to IL text,
//! which then feeds into the `weftik-il-compiler` for full HAL IR compilation.
//!
...
///
/// ```ignore
/// let il = weftik_fbd_compiler::fbd_compile(
///     "BLOCK ton1 TON\n\nton1.IN := x\nton1.PT := 500\n"
/// ).unwrap();
