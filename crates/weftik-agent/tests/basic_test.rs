//! Basic integration tests for the Weftik Supervisor crate.
//!
//! Tests config loading and process lifecycle struct creation.

use weftik_agent::config::AgentConfig;
use weftik_agent::monitor::{ManagedProcess, ProcessState};
...
children:
  - name: controller
    program: /usr/bin/weftik-controller
    args: ["--socket-path", "/tmp/test.sock"]
  - name: health-monitor
    program: /usr/bin/weftik-health
uds_path: /tmp/weftik-controller.sock
...
fn test_supervisor_config_load() {
    let dir = std::env::temp_dir().join("weftik-test-supervisor-config");
    ...
    assert_eq!(config.children[0].program, "/usr/bin/weftik-controller");
    assert_eq!(config.children[0].args, vec!["--socket-path", "/tmp/test.sock"]);
    assert_eq!(config.children[1].name, "health-monitor");
    assert_eq!(config.uds_path, "/tmp/weftik-controller.sock");
