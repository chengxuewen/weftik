//! Weftik protocol spec tests — implemented against InprocTransport.
...
use weftik_amw_inproc::{InprocFactory, InprocTransport, StaticDiscovery, create_stream_channel};
use weftik_hal_core::{
    ...
    t.register_rpc_handler(
        ...
        Box::new(|params| {
            let s = std::str::from_utf8(params)
                .map_err(|_| weftik_hal_core::HalError::Internal("utf8".into()))?;
...
fn test_s_rpc_003() {
    ...
    assert!(
        matches!(err, weftik_hal_core::HalError::Timeout { .. }),
...
fn test_s_rpc_005() {
    ...
    t.register_rpc_handler(
        "configureComponent",
        Box::new(|_params| {
            Err(weftik_hal_core::HalError::Rejected { code: 400, reason: "invalid config".into() })
        }),
    )
    .unwrap();
    let err = t.rpc_call("configureComponent", b"bad", 1000).unwrap_err();
    assert!(matches!(err, weftik_hal_core::HalError::Rejected { .. }));
}
...
fn test_s_rpc_007() {
    ...
    assert!(matches!(err, weftik_hal_core::HalError::Timeout { .. }));

    // 2. Rejected — method not found
    let err = t.rpc_call("no_handler", b"", 100).unwrap_err();
    assert!(matches!(err, weftik_hal_core::HalError::NotFound { .. }));

    // 3. Execution — handler internal failure
    t.register_rpc_handler(
        "may_fail",
        Box::new(|_params| {
            Err(weftik_hal_core::HalError::Execution {
    ...
    assert!(matches!(err, weftik_hal_core::HalError::Execution { .. }));
}
...
fn test_s_amw_002() {
    ...
    discovery
        ...
        .unwrap();
    discovery.notify_watchers(DiscoveryEvent::Added(weftik_hal_core::DiscoveryEntry {
        name: "new.sig".into(),
        description: "added".into(),
        metadata: weftik_hal_core::Metadata::default(),
        created_at: ts,
    }));
...
fn test_s_amw_003() {
    ...
    // Assert — backend name
    assert_eq!(mw.backend_name(), "weftik-amw-inproc");
...
fn test_s_type_001() {
    ...
    let test_cases: Vec<(&str, HalValue)> = vec![
        ...
        (
            "type.array",
            HalValue::Array {
                element_type: weftik_hal_core::HalPinType::S32,
...
fn test_s_type_002() {
    ...
    assert_eq!(val.pin_type(), weftik_hal_core::HalPinType::Blob);
