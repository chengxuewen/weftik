//! Weftik IEC 61131-3 LD (Ladder Diagram) compiler.
//!
//! Compiles LD source text to IL text, which then feeds into the
//! `weftik-il-compiler` for full HAL IR compilation.
//!
//! # LD text format
//!
//! ```text
...
mod tests {
    ...
    fn test_pipeline_ld_to_il_to_hal() {
        ...
        let hal_prog = weftik_il_compiler::il_compile(&il_text).unwrap();
    ...
    fn test_pipeline_execute() {
        ...
        let il_text = ld_compile(ld_src).unwrap();
        let hal_prog = weftik_il_compiler::il_compile(&il_text).unwrap();
        let mut executor = weftik_hal_ir::Executor::new(hal_prog);
