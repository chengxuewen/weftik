...
#[test]
#[ignore = "ST->IR control-flow regression (while/repeat/exit family; 12 hang + 6 fail, repro on pre-rename history). Separate fix task — see pitfalls.md 2026-09-23"]
...
fn test_ton_timer_advances_with_cycle_time() {
    ...
    let has_timer_run = program
        .instructions
        .iter()
        .any(|i| matches!(i.opcode, weftik_hal_ir::instruction::Opcode::TimerRun));
...
fn test_sr_sets_q1_on_s1() {
    ...
    let has_sr_run = program
        .instructions
        .iter()
        .any(|i| matches!(i.opcode, weftik_hal_ir::instruction::Opcode::SrRun));
