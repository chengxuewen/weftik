//! Weftik CNC Axis Group — homing, soft limits, backlash, alarm management.
//!
...
pub fn generate_combined_program(cfg: &AxisGroupConfig) -> HalProgram {
    ...
    instructions.push(weftik_hal_ir::instruction::Instruction::halt());
...
mod tests {
    ...
    fn test_alarm_state_aggregation() {
        ...
        assert_eq!(
            ...
            weftik_hal_ir::instruction::Opcode::Halt
        );
    }
    ...
    fn test_combined_program_no_duplicate_halt() {
        ...
        let halt_count = prog
            .instructions
            .iter()
            .filter(|i| i.opcode == weftik_hal_ir::instruction::Opcode::Halt)
            .count();
