/**
 * GCodeEditorTool — G-code text editor with compile & run.
 * Uses CodeEditor (st mode for syntax highlighting) with G-code compilation pipeline.
 */
...
export default function GCodeEditorTool({ toolId, eventBus }: ToolProps) {
  ...
  const handleCompileRun = useCallback(async () => {
    ...
    setSignals([]);
    try {
      const programJson: string = await invoke("compile_gcode", { source });
  ...
  const handleOpen = useCallback(async () => {
    try {
      const selected = await openFileDialog({
        ...
        filters: [{ name: "G-code", extensions: ["gcode", "nc", "ngc", "txt"] }],
        multiple: false,
      });
  ...
  return (
    <div className="app-panel app-panel--editor">
      <div className="app-panel__header">G-code Editor</div>
      ...
      <div style={{ borderTop: "1px solid var(--color-border)", padding: 4, fontSize: 11, color: "var(--color-text-tertiary)", display: "flex", justifyContent: "space-between" }}>
        <span>{shortName ?? "untitled.gcode"} | Ln {cursorLine}, Col {cursorCol}</span>
...
export const descriptor: ToolDescriptor = {
  id: "audesys.gcode-editor",
  label: "G-code Editor",
  icon: "cnc",
  group: "editor",
  component: GCodeEditorTool,
};
