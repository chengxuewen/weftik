/**
 * IlCompilerTool — IL (Instruction List) code editor with compile & run.
...
export default function IlCompilerTool({ toolId, eventBus }: ToolProps) {
  ...
  const handleCompileRun = useCallback(async () => {
    ...
    setSignals([]);
    try {
      const programJson: string = await invoke("compile_il", { source });
  ...
  const handleOpen = useCallback(async () => {
    try {
      const selected = await openFileDialog({
        title: "Open IL Source File",
        filters: [{ name: "Instruction List", extensions: ["il", "IL", "txt"] }],
        multiple: false,
      });
    ...
    try {
      if (currentFile) {
        ...
        const selected = await saveFileDialog({
          title: "Save IL Source File",
          filters: [{ name: "Instruction List", extensions: ["il"] }],
        });
        if (selected) {
  ...
  return (
    <div className="app-panel app-panel--editor">
      <div className="app-panel__header">IL Source Editor</div>
      ...
      <div style={{ borderTop: "1px solid var(--color-border)", padding: 4, fontSize: 11, color: "var(--color-text-tertiary)", display: "flex", justifyContent: "space-between" }}>
        <span>{shortName ?? "untitled.il"} | Ln {cursorLine}, Col {cursorCol}</span>
...
export const descriptor: ToolDescriptor = {
  id: "audesys.il-compiler",
  label: "IL Compiler",
  icon: "list",
  group: "editor",
  component: IlCompilerTool,
};
