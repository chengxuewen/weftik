...
> 本文档为 Weftik 项目参考文档，系统梳理 ROS/ROS2 的技术架构、通信模型、生态系统与设计理念，为 Weftik 的 Runtime 与 HAL 设计提供参考对比。
> 
> 最后更新：2026-07-13
...
ros2_control 的控制循环设计是 Weftik 的重要参考：
...
ROS2 提供了丰富的可观测性工具，对 Weftik 的 Runtime 调试有参考价值：
...
- **桥接方案**：
  ...
  - Modbus TCP 桥接
  - EtherCAT 从站模拟
- **Weftik 参考**：Weftik 的 Runtime 需要处理类似的工业协议集成
...
### 7.1 RMW 抽象层与 Weftik amw 的对比分析

ROS2 的 RMW 和 Weftik 的 amw（Weftik Middleware）在设计理念上高度相似，核心目标都是**将通信实现与上层 API 解耦**。

| 对比维度 | ROS2 RMW | Weftik amw（规划） |
|---------|---------|-------------------|
...
- **避免**：DDS 的 QoS 复杂度过高（ROS2 社区常见问题），Weftik 的 HalQoS 保持三个最小维度是正确的选择

### 7.2 通信模型对比

| Weftik 原语 | ROS2 等价原语 | 相似性 | 差异 |
|--------------|--------------|--------|------|
...
| 无直接等价 | Action（长时间目标导向任务） | — | Weftik 未将 Action 作为独立原语，通过 RPC + StreamChannel 组合实现 |

**关键差异**：
- ROS2 的 Action 是独立的一等原语，Weftik 用 RPC + StreamChannel 组合覆盖
- ROS2 的 Topic 有丰富的 QoS 配置（10+ 维度），Weftik 的 HalQoS 仅三个维度
- ROS2 通过 DDS 实现自动发现，Weftik 的 HalDiscovery 尚在设计中

### 7.3 ROS2 control 的 read→update→write 管线参考

ros2_control 的实时控制循环设计是 Weftik Runtime 的重要参考：

| Weftik 概念 | ROS2 control 对应 | 参考价值 |
|-------------|------------------|---------|
...
| 不同更新率 | `rw_rate` 参数 | 支持不同硬件组件不同频率，Weftik 可参考 |
...
| 模块 | 参考价值 | 适用性 |
|------|---------|-------|
| **tf2**（坐标变换） | 时间缓冲坐标变换库 | 高：Weftik 的 Simulator 和 Studio 可能需要坐标管理 |
| **rosbag2**（记录/回放） | 消息记录和回放 | 中：调试和测试场景有用 |
| **SROS2**（安全） | DDS 安全认证和加密 | 中：Weftik 的 HalQoS security_domain 参考 |
| **pluginlib**（插件机制） | 运行时动态加载插件 | 高：Weftik 的硬件组件和控制器加载机制可参考 |
| **Lifecycle 节点** | 4 状态管理 | 高：Weftik 的 Runtime 节点管理参考 |
| **Executor 模型** | 回调调度 | 中：D13 已确定混合线程调度策略 |
| **Launch 系统** | 多节点启动 | 中：Weftik Studio IDE 可能参考 |
| **colcon/ament** | 构建系统 | 低：Weftik 目标语言不同（Rust 核心） |

### 7.5 需要避免的 ROS2 已知问题

1. **DDS 配置复杂度**：10+ QoS 参数导致"不兼容 QoS 静默不通信"问题 → Weftik 保持 HalQoS 三个维度
2. **Executor 优先级反转**：默认 MultiThreadedExecutor 的问题 → Weftik 设计明确的混合线程调度
3. **发现风暴**：DDS O(N²) 发现在大规模节点部署时的性能瓶颈 → Weftik 的 amw 可设计更高效的发现机制
4. **Python 性能瓶颈**：rclpy 的 GIL 限制 → Weftik 的 Rust 核心 + 多语言节点设计
5. **IDL 代码生成**：.msg/.srv 文件需要编译时代码生成 → Weftik 的 FlatBuffers 方案更灵活
...
ROS2 的 Executor 模型为 Weftik 的线程调度设计提供了参考和警示：
...
Weftik 的 D13 决策（四系统混合线程调度）已结合这些经验：
...
ROS2 和 Weftik 在通信原语的设计哲学上有根本性差异：
...
- Weftik 追求**最小正交**：三个原语正交覆盖所有场景，QoS 三个维度最小够用
- 两者都为了实现解耦通信实现与上层应用，但抽象层次和复杂度不同

### 7.9 总结：Weftik 可采用的 ROS2 设计

| 可采用的 | 需改编的 | 应避免的 |
|---------|---------|---------|
| rmw 抽象层模式（amw 的参考原型） | rcl C 语言核心库（Weftik 用 Rust 实现） | DDS 的 O(N²) 发现协议 |
