...
> 状态：历史参考 — **Weftik 已于 D110（2026-08-03）完全移除 GLSP，LD/FBD 编辑器改 React Flow**。本文保留作 GLSP 产品画像与迁移教训（§13.3）参考。
...
GLSP 曾为 Weftik LD/FBD 编辑器迁移（D92）的技术基础，该迁移已随 D110 终止——本节为产品画像与历史教训，不再代表现状。
