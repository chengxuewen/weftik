...
| **M3** | Phase 3: StreamChannel + 3 种 QueuePolicy | Runtime I/O 线程 StreamChannel 消费端 | Scene Designer 骨架（SVG 画布，无 Logic） | 简单虚拟设备（Printer/Serial） | 10MB/s 生产者→消费者无丢帧 |
...
| 0.3 直接 TDD 启动 | 测试目录 + 首批测试场景提取（从 docs/modules/hal/ 各子文档） | 0.5 天 | — |
...
| 风险 | 概率 | 影响 | 缓解 |
|------|:---:|:---:|------|
| M0 基础设施搭设时间超预期 | 中 | 高 | 采用 dora-rs workspace 预制骨架，预算 5-7 天含缓冲 |
| hal-core trait 定义反复修改 | 高 | 中 | M1 前冻结 trait 接口（设计冻结门禁），接口变更需团队审核 |
| 直接 TDD 从设计规范提取测试场景存在遗漏 | 中 | 中 | 按 docs/modules/hal/ 子文档逐项提取，Peer Review 检查覆盖度 |
...
- [ ] CI/CD qa-fast 门禁全绿
- [ ] Runtime crate 可编译运行（验证 `cargo run`）
- [ ] Studio 骨架启动（验证 Tauri 窗口打开）
- [ ] 里程碑完成标准（见 §2 表格）达标
- [ ] 跨模块接口点对齐（见 §5）
