...
- [AUDESYS 架构文档](#audesys-架构文档)
  ...
  - [六、Web 迁移路径](#六web-迁移路径)
    ...
    - [2. 代码复用率](#2-代码复用率)
    - [3. 迁移策略](#3-迁移策略)
      - [Web Track 1: WebSocket 传输层](#web-track-1-websocket-传输层)
      - [Web Track 2: Web 部署模式](#web-track-2-web-部署模式)
      - [Web Track 3: 多租户 SaaS](#web-track-3-多租户-saas)
    - [4. 架构差异图](#4-架构差异图)
...
> 🟡 **迁移中** — Studio 正在从 Tauri+React 迁移到 Eclipse Theia。详见 `docs/superpowers/specs/2026-07-21-studio-theia-migration-design.md`

> ✅ **当前状态**: MVP 完成 — Tauri + CodeMirror 6 + 35 Tauri 命令 + 8 面板 + 7 种模式（6 语言+HMI 设计器）
...
| **Theia 骨架** | `apps/studio-theia/` 目录 | 🔲 待启动 |
| **napi-rs 绑定层** | `crates/audesys-theia-bridge/`（~25 函数） | 🔲 待启动 |
| **Theia Backend Service** | `theia-extensions/audesys-backend/` | 🔲 待启动 |
...
设计完成度:  ████████████████████████░░  ~90% (文档)
代码实现度:  ██████████████████████░░  ~85%
Studio 代码:  ████████████████████░░░░  ~85% (Tauri + React，含 HMI 设计器)
Theia 迁移:   ░░░░░░░░░░░░░░░░░░░░░░░░░░  ~0% (设计完成，待实施)
...
└──────────────────────────────┬──────────────────────────────────────┘
                               │
               ┌───────────────┴───────────────┐
               │  Rust Runtime (napi-rs 桥接)   │
               │  ┌─────────────────────────┐  │
               │  │ Controller (5-step RT)  │  │
               │  │ Supervisor (进程编排)   │  │
               │  │ IPC Server (UDS 17方法) │  │
               │  │ Modbus/HART 适配器      │  │
               │  │ SimulationHarness       │  │
...
```
┌─────────────────────────────────────────────────────────────┐
│            Layer 1: Theia Frontend（渲染进程）                │
│  ┌──────────────┐ ┌──────────┐ ┌──────────────┐            │
│  │ Electron     │ │ Browser  │ │ 嵌入模式     │            │
│  │ (桌面应用)   │ │ (Web访问)│ │ (Panel内嵌)  │            │
│  └──────┬───────┘ └────┬─────┘ └──────┬───────┘            │
│         └──────────────┴──────────────┘                     │
│                      │                                       │
...
├─────────────────────────────────────────────────────────────┤
│            Layer 2: Theia Backend（Node.js 服务）            │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ Casdoor  │  │ RBAC引擎 │  │ 项目管理 │  │ 插件系统 │   │
│  │ IdP      │->│ Action+  │->│ Project  │->│ VS Code  │   │
...
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ Compiler │  │ Runtime  │  │ IPC      │  │ Protocol │   │
│  │ (6语言+) │  │ Engine   │  │ Server   │  │ Adapters │   │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
│  (35K 行 Rust，napi-rs 编译为 .node 二进制)                  │
└─────────────────────────────────────────────────────────────┘
```
...
| **ST/IL/G-code** | text | 文本编程语言 | **Monaco Editor**（Theia 内置）+ 自定义语言服务器 | 🔄 待迁移 |
| **LD** | graph | 梯形图编程 | **Eclipse GLSP** + Rust 布局引擎 | 🔄 待迁移 |
| **FBD** | graph | 功能块图编程 | **Eclipse GLSP** + Rust 布局引擎 | 🔄 待迁移 |

> **已实现编辑器（当前 Tauri MVP）**: ST/IL/LD/FBD/SFC (CodeMirror 6)、G-code (CodeMirror 6)、HMI 可视化设计器 (react-rnd + SVG widget)。
> **Theia 迁移策略**: ST/IL/G-code → Monaco Editor（Theia 内置），LD/FBD → Eclipse GLSP 图形编辑器，HMI → ReactWidget 包装。
...
│  │ 35 Tauri 命令: save_hmi_layout / load_hmi_layout         │    │
...
| **SignalBindingDialog** | `SignalBindingDialog.tsx` (118行) | 模态框搜索可用 HAL 信号 → `controller_signal_snapshot` Tauri 命令 |
...
| **useHmiSignal hook** | `useHmiSignal.ts` (46行) | 500ms 定时器轮询 `controller_read_signal` Tauri 命令 |
...
| 缺口 | 影响 | 优先级 |
|------|------|:----:|
| **信号值注入** | Preview 模式下无法模拟信号变化，必须连接真实 Controller。`sim_set_signal` Tauri 命令存在但未集成到 HMI Builder | 🔴 |
...
│ Step 1: 身份验证 (Casdoor OIDC)                  │
  JWT (Studio Web 前端 OIDC) / HMAC Token (Runtime IPC UDS + SO_PEERCRED) -> userId: 'alice', role: 'developer'
│  ✅ platform:login -> pass                        │
├──────────────────────────────────────────────────┤
│ Step 2: Studio 访问权限                          │
...
AUDESYS Studio 采用四层插件架构，参考 VS Code Extension API，支持 PC (Tauri) / Web 双模式：
...
│  │              │  │                  │  │              │    │
│  │ · manifests  │  │ · 37 commands    │  │ · layout     │    │
│  │ · lifecycle  │  │ · 7 namespaces   │  │ · slots      │    │
│  │ · activate   │  │ · keybindings    │  │ · docking    │    │
│  └──────┬───────┘  └────────┬─────────┘  └──────┬───────┘    │
│         │                   │                   │            │
│         └───────────────────┼───────────────────┘            │
│                             │                                │
...
模仿 VS Code 的 `activate` / `deactivate` 模式：

```typescript
interface PluginManifest {
  id: string;                    // "audesys.st-compiler"
  displayName: string;
  version: string;
  engines: { studio: string };
  main: string;                  // Node.js/ES module entry
  browser?: string;              // Web mode override (dual-platform)
...
**25 种激活事件**（按需延迟加载，参考 VS Code）：

| 事件 | 触发条件 | 示例 |
|------|---------|------|
| `onLanguage:st` | 打开 ST 文件 | ST 编译器/编辑器模块 |
| `onCommand:compiler.st.compile` | 执行编译命令 | 命令 handler 注册 |
| `onView:debug` | 展开调试面板 | DAP 面板组件 |
| `onStartupFinished` | Shell 启动完成 | 核心面板延迟注册 |
| `workspaceContains:**/*.st` | 工作区包含 ST 文件 | 项目类型检测 |

**生命周期**: Manifest Scan → Contribution Index → UI Bootstrap → Trigger activationEvent → `import()` + `activate()` → Running → `deactivate()`

#### 8.2 Command Registry

37 个命令，7 个命名空间（参考 VS Code CommandsRegistry priority-linked-list 模式）：

| 命名空间 | 命令数 | 示例 |
|----------|:---:|------|
| `compiler.*` | 8 | compiler.st.compile, compiler.gcode.compile |
| `project.*` | 4 | project.open, project.create |
| `deploy.*` | 2 | deploy.program, deploy.hal_config |
| `signal.*` | 3 | signal.read, signal.snapshot |
| `controller.*` | 11 | controller.debug.pause/resume/step |
| `sim.*` | 7 | sim.control.start/stop, sim.modbus.* |
| `hmi.*` | 2 | hmi.layout.save/load |

PC 模式通过 `PlatformAdapter.invoke()` 调用 Tauri 后端；Web 模式通过 `fetch('/api/invoke')` 调用 HTTP API。
...
`IPlatformAdapter` 是唯一的 `@tauri-apps/*` 导入点。PC 模式封装 Tauri API，Web 模式使用浏览器 API (fetch + IndexedDB + File API)。

| 能力 | PC (Tauri) | Web |
|------|:---:|:---:|
| 文件系统 | Tauri FS plugin | IndexedDB VFS |
| RPC 调用 | `invoke()` → Rust | `fetch("/api/invoke")` → HTTP |
| 文件对话框 | 原生 OS 对话框 | `<input type="file">` |
| 终端 | PTY ✅ | ❌ 隐藏 |
...
| 风险 | 影响 | 概率 | 对策 |
|------|:----:|:----:|------|
| **Casdoor 集成复杂度** | 高 | 中 | Phase 1 Studio Web 用内置 JWT（OIDC），Runtime IPC 用 HMAC（已实现）。Phase 2 后期接 Casdoor |
...
### 5. PlatformAdapter 驱动的 Web 迁移

通过 `IPlatformAdapter` 接口，前端组件零修改即可支持 PC/Web 双模式：

```
PC:  PcAdapter.invoke("compile_st", args)  → Tauri invoke → Rust backend
Web: WebAdapter.invoke("compile_st", args)  → fetch('/api/invoke') → same Rust backend
```

| 模块 | PC (Tauri) | Web | 适配策略 |
|------|-----------|-----|---------|
| RPC 调用 | `invoke()` | `fetch()` | 相同签名 |
| 文件系统 | Tauri FS plugin | IndexedDB VFS | 透明适配 |
| 文件对话框 | 原生 OS | `<input type="file">` | 功能等价 |
| 终端 | PTY | ❌ 隐藏 | capabilities.nativeProcess=false |
| 调试 | UDS → DAP | WebSocket → DAP | 协议不变 |
| 仿真器 | child_process.fork | Docker 容器 | Phase 2+ |

P1 已实现 PcAdapter（5 文件），WebAdapter 骨架已设计。
详见 `docs/modules/studio/plugin-architecture-design.md` §3 和 `docs/reference/vscode.md`。
...
| 源语言 | 编译器 Crate | 输出类型 |
|--------|-------------|----------|
| ST | `audesys-st-compiler` | HalProgram |
