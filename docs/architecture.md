...
`IPlatformAdapter` 是唯一的 `@tauri-apps/*` 导入点。PC 模式封装 Theia Backend (napi-rs bridge)，Web 模式使用浏览器 API (fetch + IndexedDB + File API)。

| 能力 | PC (Theia) | Web |
|------|:---:|:---:|
| 文件系统 | Theia FileService | IndexedDB VFS |
| RPC 调用 | napi-rs bridge → Rust | `fetch("/api/invoke")` → HTTP |
...
├──────────┼──────────────────────────────────────────────────┤
│  桌面端  │  Theia (Electron) 打包                          │
│          │  适合: 离线开发、低延迟、本地文件系统访问         │
│          │  技术: Theia + Monaco + napi-rs bridge           │
...
Theia Backend (napi-rs):  Electron: 原生运行；Browser: token-patch.py + polyfill (已实现)
```
...
| Theia Backend (napi-rs) | Electron 100% / Browser polyfill | token-patch.py + 38 API polyfill（已完成） |
...
```
桌面版架构:
  Studio (Theia) ←UDS-> Controller (Rust) ←Serial-> 设备
  Studio (Theia) ←UDS-> Simulator (fork)
...
────────── ⚠️ 已弃用（D71 替代）以下为旧 Tauri PlatformAdapter 设计 ──────────

通过 `IPlatformAdapter` 接口，前端组件零修改即可支持 PC/Web 双模式。D58 的四层插件架构已被 Theia Extension System 取代。以下为历史记录：

通过 `IPlatformAdapter` 接口，前端组件零修改即可支持 PC/Web 双模式：

```
PC:  PcAdapter.invoke("compile_st", args)  → Tauri invoke (旧) → Rust backend
Web: WebAdapter.invoke("compile_st", args)  → fetch('/api/invoke') → same Rust backend

Theia 等效: napi-rs bridge (21/30 函数) + Theia Backend Service (JSON-RPC proxy)
Web: WebAdapter.invoke("compile_st", args)  → fetch('/api/invoke') → same Rust backend
```

| 模块 | PC (Theia Electron) | Web | 适配策略 |
|------|-----------|-----|---------|
| RPC 调用 | napi-rs bridge | `fetch()` → Theia Backend | 相同 Rust 后端 |
| 文件系统 | Theia FileService | IndexedDB VFS | 透明适配 |
...
P1 已实现 Theia Backend napi-rs bridge (21/30 函数)。Web 浏览器模式通过 token-patch.py + postbuild.sh 实现 38 API polyfill，Electron + 浏览器双端可用。详见 `docs/superpowers/specs/2026-07-21-studio-theia-migration-design.md`。
