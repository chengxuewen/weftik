...
> ✅ **状态**: MVP 完成 — Tauri + CodeMirror 6 + 35 Tauri 命令 + 8 面板 + 7 种模式（6 语言+HMI 设计器）
...
| 编辑器 | 类型 | 用途 | 技术 | 状态 |
|--------|------|------|------|:----:|
| **Scene Designer** | scene | 画面组态（HMI/Panel 界面设计） | react-rnd + SVG | ✅ 已实现 |
...
> **已实现编辑器（MVP）**: ST/IL/LD/FBD/SFC (CodeMirror 6)、G-code (CodeMirror 6)、HMI 可视化设计器 (react-rnd + SVG widget)。
> **愿景编辑器**: Flow Designer (DAG 流程, @xyflow/react)、Data Designer (数据模型, Drizzle schema)、App Builder (无代码)。
> **编译器策略变更（D56/D57）**: RuSTy 方案已废弃。当前为自研 6 种编译器（5 IEC + G-code），全部编译到 HalProgram 后端。
