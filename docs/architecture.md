...
- [AUDESYS 概述](#audesys-概述)
  ...
  - [产品线](#产品线)
  - [术语速查](#术语速查)
- [一、Runtime 套件模块设计](#一runtime-套件模块设计)
  ...
  - [14. 模块分类矩阵](#14-模块分类矩阵)
  - [15. 部署模式](#15-部署模式)
- [二、AUDESYS Studio](#二audesys-studio)
  - [1. 产品定位](#1-产品定位)
  - [2. 现状评估](#2-现状评估)
...
- [三、HAL 协议规范](#三hal-协议规范)
  ...
  - [7. 传输模式演进](#7-传输模式演进)
- [四、HAL Core (Rust)](#四hal-core-rust)
  - [1. 设计理念 - Sans-I/O](#1-设计理念---sans-io)
  - [2. Rust HAL Core 接口](#2-rust-hal-core-接口)
  - [3. Controller 内部架构 - RT 线程 + I/O 线程](#3-controller-内部架构---rt-线程--io-线程)
  - [4. 驱动注册表](#4-驱动注册表)
  - [5. SerialGCodeDriver 实现示例](#5-serialgcodedriver-实现示例)
- [五、多语言 Thin Client](#五多语言-thin-client)
  - [1. 设计理念 - Thin Client](#1-设计理念---thin-client)
  - [2. Thin Client 接口规范](#2-thin-client-接口规范)
  - [3. 语言绑定对比](#3-语言绑定对比)
  - [4. Node.js Thin Client（napi-rs）](#4-nodejs-thin-clientnapi-rs)
  - [5. Python Thin Client（PyO3）](#5-python-thin-clientpyo3)
  - [6. C/C++ Thin Client](#6-cc-thin-client)
  - [7. 现有代码迁移策略](#7-现有代码迁移策略)
  - [8. 语言选择指南](#8-语言选择指南)
- [六、模拟器系统](#六模拟器系统)
...
**AUDESYS**（**Au**tomation **De**velopment **Sys**tem）是一个面向工业设备控制的自动化开发系统，专注于实时控制、硬件抽象层（HAL）、设备仿真和工业调试。
...
技术栈方面，AUDESYS 采用 Rust/C/C++ 实现实时控制（子毫秒确定性），Node.js 实现 Supervisor 进程管理，Python 实现 Gateway 协议网关。整体架构灵感来自 Android Studio 的"IDE + Runtime + Emulator + Debug Bridge"分层模型。

### 产品线

| 产品 | 描述 | 技术栈 | 状态 |
|------|------|--------|------|
| AUDESYS Studio | 统一编辑器/IDE | TypeScript + React + React Flow | 🔮 规划中 |
...
| 术语 | 说明 |
|------|------|
| **AUDESYS Studio** | 统一编辑器/IDE，可视化配置设备模板、流程逻辑和 HMI 界面 |
| **AUDESYS Runtime** | PC 应用套件，多进程实时控制系统 |
| **Supervisor** | 进程/参数/配置/更新管理器（Node.js） |
| **Controller** | 实时控制模块（Rust + dora-rs），旧名 Act |
| **Panel** | 触摸屏 HMI 人机交互（Tauri + React），旧名 HMI |
| **Gateway** | 外部通信应用（MES/ERP/云对接） |
| **Remote** | 远程访问（WebRTC 浏览器 B/S 架构） |
| **Edge** | 边缘采集（7×24 独立运行） |
| **AUDESYS Link** | 通信中间件库/SDK |
| **HAL** | 硬件抽象层（Hardware Abstraction Layer） |
| **HAL Core** | HAL 核心实现（Rust，Sans-I/O） |
| **Thin Client** | 多语言 HAL 客户端绑定（Node.js/Python/C++） |
...
借鉴 LinuxCNC HAL 的 Pin-Component-Function-Thread 模型：

```rust
struct HalPin<T> { name, value, direction, signal }
trait HalComponent { name(), pins(), functions() }
struct HalFunction { name, period_ns, priority, fp }
struct HalThread { name, period_ns, functions, cpu_affinity }
```
...
### 3. Controller 内部架构 - RT 线程 + I/O 线程
