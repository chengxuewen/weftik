...
cargo test -p weftik-ld-compiler --lib -q 2>/dev/null
echo "  ✅ LD compiler: 31/31"

# 2. IL compiler  
echo "[2/4] IL compiler..."
cargo test -p weftik-il-compiler --lib -q 2>/dev/null
echo "  ✅ IL compiler: 32/32"

# 3. Agent (renamed from Supervisor)
echo "[3/4] Agent..."
cargo test -p weftik-agent -q 2>/dev/null
echo "  ✅ Agent: 3/3"

# 4. LD→Runtime pipeline (deploy + execute + verify)
echo "[4/4] LD→Runtime pipeline..."
cargo test -p weftik-runtime --test pipeline_test -q 2>/dev/null
echo "  ✅ Pipeline: 7/7"
