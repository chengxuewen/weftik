 ...
 *
 * The frontend cannot load the native `@weftik/theia-bridge` (browser
 * mode), so ST compilation is routed here over JSON-RPC. Mirrors the LD
 * editor's backend module (loadBridge + compile call).
 *
 * The esbuild bundle maps `@weftik/theia-bridge` to a path STRING (the
...
function loadBridge() {
    ...
    try {
        ...
        files = fs.readdirSync(nativeDir).filter((f) => f.endsWith('.node') && (f.includes('weftik') || f.startsWith('index.')));
    }
    catch {
    ...
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    return require('@weftik/theia-bridge');
}
exports.default = new inversify_1.ContainerModule((bind) => {
    bind(common_1.ConnectionHandler).toDynamicValue(() => new common_1.JsonRpcConnectionHandler(st_compile_protocol_1.StCompileServicePath, () => {
        ...
        const deployProject = (programs) => {
            ...
            const socket = process.env.WEFTIK_SOCKET ?? '/tmp/weftik-runtime.sock';
            const secret = process.env.WEFTIK_SECRET ?? '';
