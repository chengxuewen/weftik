...
export const POU_TYPES: Readonly<PouType[]> = ['Program', 'FunctionBlock', 'Function', 'GVL'];

const PROGRAM_LANGUAGES: Readonly<PouLanguage[]> = ['ST', 'IL'];
 ...
 */
export function languagesFor(type: PouType): Readonly<PouLanguage[]> {
...
function templateFor(type: PouType, language: PouLanguage): (name: string) => string {
    switch (type) {
        case 'Program':
            return language === 'ST'
...
function dirFor(type: PouType): string {
    switch (type) {
...
function extFor(language: PouLanguage): string {
    switch (language) {
        case 'ST': return '.st';
        case 'IL': return '.il';
        case 'GVL': return '.gvl';
...
export function pouTarget(type: PouType, language: PouLanguage): PouTarget {
