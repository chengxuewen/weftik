...
class LdContactView {
    render(model, context) {
        ...
        return (0, snabbdom_1.h)('g', { attrs: { id: model.id, 'data-element-id': model.id } }, children);
    }
}
...
class LdPowerRailView {
    render(model, context) {
        ...
        const totalHeight = Math.max((model.size?.height || 400) - CANVAS_PADDING, 400);
...
class LdFbView {
    render(model, context) {
        ...
        const w = 120;
        const h = Math.max(40 + (getElementProp(model, 'pinCount', 4)) * 16, 60);
        const isSelected = getElementProp(model, 'selected', false);
        const children = [];
        // Selection highlight
        if (isSelected) {
 ...
 * We check both.
 */
function getElementProp(model, prop, defaultValue) {
...
//# sourceMappingURL=ld-views.js.map