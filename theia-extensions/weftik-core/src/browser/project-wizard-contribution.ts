...
export namespace ProjectWizardCommands {
    export const CATEGORY = 'Weftik';

    export const NEW_PROJECT: Command = {
        id: 'weftik.new.project',
        label: 'New Weftik Project…',
        category: CATEGORY,
    };
}

/**
 * New Weftik Project wizard (A-工程管理).
 * Creates the standard directory convention (Programs/FBs/Functions/GVL) plus a
 * lightweight `project.yaml` manifest.
 *
 * Two modes:
 *  - No workspace open → picks a parent directory (default ~/Weftik-Projects/,
...
export class ProjectWizardContribution implements CommandContribution, MenuContribution {
    ...
    protected async runWizard(): Promise<void> {
        ...
        try {
            ...
            this.messageService.error(`Failed to create Weftik project: ${String(e)}`);
            return;
        }

        if (created.length > 0) {
            this.messageService.info(`Weftik project "${name}" created: ${created.join(', ')}`);
        }
        if (skipped.length > 0) {
    ...
    /** Pick a parent dir (default ~/Weftik-Projects/), create {name}/, return its URI. */
    ...
    /** The default projects parent dir: {home}/Weftik-Projects/. */
    ...
    protected async pickName(): Promise<string | undefined> {
        return this.quickInput.input({
            title: 'New Weftik Project',
            prompt: 'Name of the Weftik project (used in project.yaml)',
