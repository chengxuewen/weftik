...
(function (IecContextMenuCommands) {
    IecContextMenuCommands.CATEGORY = 'Weftik';
...
let IecContextMenuContribution = class IecContextMenuContribution {
    ...
    registerMenus(menus) {
        ...
        menus.registerMenuAction(['navigator-context-menu', 'weftik-group'], {
    ...
    async onCompile() {
        ...
        let raw;
        try {
            const bridge = require('@weftik/theia-bridge');
    ...
    getSocketPath() {
        ...
        return process.env.WEFTIK_SOCKET ?? '/tmp/weftik-controller.sock';
    }
    getSecret() {
        // ponytail: env var or local-dev default, add secure keychain in P2
        return process.env.WEFTIK_HMAC_SECRET ?? 'weftik-dev-secret';
    }
};
