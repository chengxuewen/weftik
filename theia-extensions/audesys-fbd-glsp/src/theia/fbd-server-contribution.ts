/**
 * FBD Server Contribution — launches the FBD GLSP server as a socket process.
 *
...
const PORT_ARG_KEY = 'FBD_GLSP';

@injectable()
export class FbdServerContribution extends GLSPSocketServerContribution {
    readonly id = FbdDiagramLanguage.contributionId;

    createContributionOptions(): Partial<GLSPSocketServerContributionOptions> {
        return {
            executable: require.resolve('audesys-fbd-glsp/lib/server/fbd-server-index'),
