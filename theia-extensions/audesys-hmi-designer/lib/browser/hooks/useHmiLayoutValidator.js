...
function validateLayout(layout, options) {
    ...
    if (count === 0) {
        ...
        warnings.push(`widget count ${count} exceeds recommended limit of ${WARN_WIDGETS}`);
    }
    const seenIds = new Set();
    for (const w of layout.widgets) {
        const prefix = w.id ? `widget '${w.id}'` : `widget at index ${layout.widgets.indexOf(w)}`;
        if (!w.id)
            errors.push(`widget missing required field 'id'`);
        if (!w.type)
...
//# sourceMappingURL=useHmiLayoutValidator.js.map