...
import { WeftikDebugChannel, IDebugBridge } from '../browser/debug-channel';
...
function makeChannel(bridge?: MockBridge) {
  ...
  const ch = new WeftikDebugChannel({ socketPath: '/tmp/test.sock', secret: 'test-secret' }, b);
...
function send(channel: WeftikDebugChannel, command: string, args?: unknown, seq?: number): void {
...
describe('WeftikDebugChannel', () => {
  let channel: WeftikDebugChannel;
