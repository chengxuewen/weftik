/**
 * RealDapBridge — spawns the `weftik-dap-adapter` binary and drives it as a DAP
...
const DEFAULT_ADAPTER_PATH = 'target/debug/weftik-dap-adapter';
...
export class RealDapBridge implements IDebugBridge {
    ...
    constructor(adapterPath?: string) {
        this.adapterPath = adapterPath ?? process.env.WEFTIK_DAP_ADAPTER ?? DEFAULT_ADAPTER_PATH;
    }
...
export function resolveAdapterPath(baseDir: string): string {
    const env = process.env.WEFTIK_DAP_ADAPTER;
    if (env) {
        return env;
    }
