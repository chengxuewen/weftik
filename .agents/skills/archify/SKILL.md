...
> 来源: vendored from github.com/tt-a1i/archify@9e35d2b (MIT, LICENSE/THIRD_PARTY_NOTICES 随附)。2026-09-24 ecosystem-scan 引入，已排除 examples/（~8MB 演示产物）。
> 安全审查结论：child_process 仅本地 spawn（预览/导出）；网络字符串 = 127.0.0.1 本地服务 + 品牌文档链接，无外传面。首次运行前对 bin/archify.mjs 实际发起的命令保持目视。
