...
## D98: Theia target = browser（PC + Web 共存）
- **日期**: 2026-07-28
- **决定**: `package.json` 中 `theia.target` 从 `"electron"` 改为 `"browser"`。前端使用 `browser-menu-module`（Web 渲染菜单），PC 和 Web 共用同一构建产物。
- **理由**: (a) electron target 只加载 electron-menu-module，浏览器模式下菜单不渲染；(b) browser target 的 Web 菜单在 Electron 壳中也能正常工作；(c) 实现思源笔记式 PC+Web 共存：同一 kernel（main.js），Electron 只是薄壳
